(function (global) {
  var GreenEnemy = global.GreenEnemy = function (state, pirate) {
    ...
    Phaser.Sprite.call(this, state.game, x, y, 'greenEnemy'); // Call Green Enemy sprite.
    ...
    this.events.onInputDown.add(this.killGreenEnemy, this); // Kill Green Enemy on input down.
  ...
  // Set up constructor.
  GreenEnemy.prototype = Object.create(Phaser.Sprite.prototype);
  GreenEnemy.prototype.constructor = GreenEnemy;

  // Loop througout game.
  GreenEnemy.prototype.update = function () {
    // Check if Green Enemy is dead.
    if (!this.alive) {
      this.destroy(); // Destroy Green Enemy.
    }
  };

  GreenEnemy.prototype.killGreenEnemy = function () {
    ...
    if (this.health <= 0) {
      ...
      // Green Enemy is moving from right to left.
      ...
      // Green Enemy is sailing from left to right.
      else {
        // Stop the pirate, must be here, otherwise this.body.velocity.x would be always 0.
        this.body.velocity.x = 0;
        this.body.velocity.y = 0;
      ...
      this.state.punctuateGreenEnemy(); // Update HUD.
      this.state.playSoundDeadEnemy(); // Play dead enemy sound.
    }
  };

  GreenEnemy.prototype.boundOut = function (pirate) {
  ...
  GreenEnemy.prototype.rand = function (wrap) {
    var x, y, r;
