(function (global) {
  var RedEnemy = global.RedEnemy = function (state, pirate) {
    ...
    Phaser.Sprite.call(this, state.game, x, y, 'redEnemy'); // Call Red Enemy sprite.
    ...
    this.events.onInputDown.add(this.killRedEnemy, this); // Kill Red Enemy on input down.
  ...
  // Set up constructor.
  RedEnemy.prototype = Object.create(Phaser.Sprite.prototype);
  RedEnemy.prototype.constructor = RedEnemy;

  // Loop througout game.
  RedEnemy.prototype.update = function () {
    // Check if Red Enemy is dead.
    if (!this.alive) {
      this.destroy(); // Destroy Red Enemy.
    }
  };

  RedEnemy.prototype.killRedEnemy = function () {
    ...
    if (this.health <= 0) {
      ...
      // Red Enemy is moving from right to left.
      ...
      // Red Enemy is sailing from left to right.
      else {
        // Stop the pirate, must be here, otherwise this.body.velocity.x would be always 0.
        this.body.velocity.x = 0;
        this.body.velocity.y = 0;
      ...
      this.state.punctuateRedEnemy(); // Update HUD.
      this.state.playSoundDeadEnemy(); // Play dead enemy sound.
    }
  };

  RedEnemy.prototype.boundOut = function (pirate) {
  ...
  RedEnemy.prototype.rand = function (wrap) {
    var x, y, r;
