(function (global) {
  var WhiteEnemy = global.WhiteEnemy = function (state, pirate) {
    ...
    Phaser.Sprite.call(this, state.game, x, y, 'whiteEnemy'); // Call White Enemy sprite.
    ...
    this.events.onInputDown.add(this.killWhiteEnemy, this); // Kill White Enemy on input down.
  ...
  // Set up constructor.
  WhiteEnemy.prototype = Object.create(Phaser.Sprite.prototype);
  WhiteEnemy.prototype.constructor = WhiteEnemy;

  // Loop througout game.
  WhiteEnemy.prototype.update = function () {
    // Check if White Enemy is dead.
    if (!this.alive) {
      this.destroy(); // Destroy White Enemy.
    }
  };

  WhiteEnemy.prototype.killWhiteEnemy = function () {
    ...
    if (this.health <= 0) {
      ...
      // White Enemy is moving from right to left.
      ...
      // White Enemy is sailing from left to right.
      else {
        // Stop the pirate, must be here, otherwise this.body.velocity.x would be always 0.
        this.body.velocity.x = 0;
        this.body.velocity.y = 0;
      ...
      this.state.punctuateWhiteEnemy(); // Update HUD.
      this.state.playSoundDeadEnemy(); // Play dead enemy sound.
    }
  };

  WhiteEnemy.prototype.boundOut = function (pirate) {
  ...
  WhiteEnemy.prototype.rand = function (wrap) {
    var x, y, r;
